...
declare -a PKGS=(
    ...
    "libpatch/logbridge"
    "libpatch/logbridge/httpadmin"
    "libpatch/cryptosuitebridge"
    "libpatch/cachebridge"
...
declare -a FILES=(
    ...
    "libpatch/logbridge/logbridge.go"
    "libpatch/logbridge/httpadmin/spec.go"
    "libpatch/cryptosuitebridge/cryptosuitebridge.go"
    "libpatch/cachebridge/cache.go"
...
FILTER_FILENAME=""libinternal/configtxgen/encoder/encoder.go""
sed -i'' -e '/^import / a \
"github.com\/golang\/protobuf\/proto"' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"

FILTER_FILENAME=""gossip/protoext/stringers.go""
sed -i'' -e '/^import / a \
"github.com\/golang\/protobuf\/proto"' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"

FILTER_FILENAME=""gossip/protoext/signing.go""
sed -i'' -e '/^import / a \
"github.com\/golang\/protobuf\/proto"' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"

FILTER_FILENAME=""protoutil/signeddata.go""
sed -i'' -e '/^import / a \
"github.com\/golang\/protobuf\/proto"' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"

FILTER_FILENAME="bccsp/pkcs11/impl.go"
sed -i'' -e '/"github.com\/hyperledger\/fabric\/bccsp"/a sdkp11 "github.com\/trustbloc\/fabric-lib-go-ext\/pkg\/core\/cryptosuite\/common\/pkcs11"' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
...
sed -i'' -e '/"math\/big"/a "github.com\/trustbloc\/fabric-lib-go-ext\/internal\/github.com\/hyperledger\/fabric\/libpatch\/cachebridge"' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
sed -i'' -e '/"math\/big"/a sdkp11 "github.com\/trustbloc\/fabric-lib-go-ext\/pkg\/core\/cryptosuite\/common\/pkcs11"' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
...
' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
sed -i'' -e 's/\"go.uber.org\/zap\/zapcore/logging\"github.com\/trustbloc\/fabric-lib-go-ext\/internal\/github.com\/hyperledger\/fabric\/libpatch\/logbridge/g' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
...
"github.com\/hyperledger\/fabric\/libpatch\/cryptosuitebridge"\
' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
...
"github.com\/trustbloc\/fabric-lib-go-ext\/pkg\/common\/providers\/core"' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
sed -i'' -e 's/bccsp.BCCSP/core.CryptoSuite/g' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
...
"github.com\/trustbloc\/fabric-lib-go-ext\/pkg\/common\/providers\/core"\
' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
sed -i'' -e 's/"github.com\/hyperledger\/fabric\/bccsp"/bccsp "github.com\/trustbloc\/fabric-lib-go-ext\/internal\/github.com\/hyperledger\/fabric\/libpatch\/cryptosuitebridge"/g' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
sed -i'' -e 's/bccsp.Key/core.Key/g' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
...
sed -i'' -e 's/"github.com\/hyperledger\/fabric\/bccsp\/factory"/factory "github.com\/trustbloc\/fabric-lib-go-ext\/internal\/github.com\/hyperledger\/fabric\/libpatch\/cryptosuitebridge"/g' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
sed -i'' -e 's/bccsp.BCCSP/core.CryptoSuite/g' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
sed -i'' -e 's/bccsp.Key,/core.Key,/g' "${TMP_PROJECT_PATH}/${FILTER_FILENAME}"
