...
# FABRIC_LIB_GO_EXT_CODELEVEL_TAG: Go tag that represents the fabric code target
# FABRIC_LIB_GO_EXT_CODELEVEL_VER: Version that represents the fabric code target (primarily for fixture lookup)
# FABRIC_LIB_GO_EXT_TESTRUN_ID: An identifier for the current run of tests.
# FABRIC_CRYPTOCONFIG_VERSION: Version of cryptoconfig fixture to use

set -e

GO_CMD="${GO_CMD:-go}"
FABRIC_LIB_GO_EXT_CODELEVEL_TAG="${FABRIC_LIB_GO_EXT_CODELEVEL_TAG:-devstable}"
FABRIC_LIB_GO_EXT_TESTRUN_ID="${FABRIC_LIB_GO_EXT_TESTRUN_ID:-${RANDOM}}"
...
echo "Code level ${FABRIC_LIB_GO_EXT_CODELEVEL_TAG} (Fabric ${FABRIC_LIB_GO_EXT_CODELEVEL_VER})"
echo "Running unit tests..."
GO_TAGS="${GO_TAGS} ${FABRIC_LIB_GO_EXT_CODELEVEL_TAG}"

GO_LDFLAGS="${GO_LDFLAGS} -X ${PROJECT_MODULE}/test/metadata.ProjectPath=${PROJECT_DIR}"
GO_LDFLAGS="${GO_LDFLAGS} -X ${PROJECT_MODULE}/test/metadata.ChannelConfigPath=test/fixtures/fabric/${FABRIC_LIB_GO_EXT_CODELEVEL_VER}/channel"
...
GO_LDFLAGS="${GO_LDFLAGS} -X ${PROJECT_MODULE}/test/metadata.TestRunID=${FABRIC_LIB_GO_EXT_TESTRUN_ID}"
